...
tgtadm_err account_add(char *user, char *password)
{
	...
	ac->aid = aid;
	ac->user = pcs_strdup(user);
	if (!ac->user)
		goto free_account;

	ac->password = pcs_strdup(password);
	if (!ac->password)
		goto free_username;
...
tgtadm_err tgt_target_create(int lld, int tid, char *args)
{
	...
	target->name = pcs_strdup(targetname);
	if (!target->name) {
		pcs_free(target);
		return TGTADM_NOMEM;
	}
